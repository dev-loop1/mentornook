...
async function loadConnections() {
    ...
    if (!incomingList || !outgoingList || !currentList || !loadingIncoming || !loadingOutgoing || !loadingCurrent || !noIncoming || !noOutgoing || !noCurrent) {
        console.error("loadConnections: One or more required list/message elements not found on the page.");
        return;
    }
    ...
    } catch (error) { // Catch network/fetch errors or errors during rendering
        ...
        items.forEach(item => {
            try {
                ...
                if (typeof createConnectionItem === 'function') {
                     ...
                     listElement.innerHTML = '<p class="error-message">Error rendering list items.</p>';
...
function createConnectionItem(item, type, loggedInUserId) {
    ...
    if (requesterIdStr === loggedInUserIdStr) { // If logged-in user sent the request
        otherUser = item.receiver;
        otherUserProfile = item.receiver_profile;
    } else if (receiverIdStr === loggedInUserIdStr) { // If logged-in user received the request
        otherUser = item.requester;
        otherUserProfile = item.requester_profile;
    } else { // Fallback if user data is unexpected
...
async function handleConnectionListPageAction(event) {
    ...
    siblingButtons.forEach(btn => { if(btn !== button) btn.disabled = true; }); // Disable siblings too
    ...
    try {
        ...
        const response = await WorkspaceApi(`/connections/${connectionId}/`, method, data, true); // requiresAuth = true
        ...
        if (response.success || response.status === 204) { // Treat 204 No Content as success for DELETE
            ...
            if (!loggedInUser) throw new Error("Cannot update UI without logged-in user info.");
            ...
            if (action === 'accept') {
                ...
                if (currentList && response.data && typeof createConnectionItem === 'function') {
                    ...
                    checkIfListEmpty('current-connections-list'); // Update 'current' list's empty state
                     ...
                     console.warn("Accept successful, but no updated connection data returned. 'Current' list may not be updated until next full load.");
...
function checkIfListEmpty(listId) {
    ...
    if (!listElement || !listSection) {
        console.warn(`checkIfListEmpty: Could not find list element or parent section for ID: ${listId}`);
        return;
    }
    ...
    if (!noItemsMessage) {
         console.warn(`checkIfListEmpty: Could not find 'no items' message element within section for list ID: ${listId}`);
        return;
    }
