document.addEventListener('DOMContentLoaded', () => {
    ...
    if (mobileMenuToggle && mainNav) {
        mobileMenuToggle.addEventListener('click', () => {
            ...
            mobileMenuToggle.textContent = mainNav.classList.contains('mobile-nav-active') ? '✕' : '☰';
        });
    }
...
if (header) {
    header.addEventListener('click', async (event) => { // Make handler async
        if (event.target && event.target.id === 'logout-button') {
            ...
            try {
                ...
                if (!response.success) {
                    ...
                    console.warn('Backend logout failed (maybe token was already invalid?):', response.error);
                }
            } catch (error) {
...
function handleAuthState() {
    ...
    const protectedPages = ['dashboard.html', 'profile_setup.html', 'my_profile.html', 'user_profile.html', 'connections.html'];
